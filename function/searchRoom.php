                    ...
                    <table class="table table-striped" style="width:100%">
                        ...
                        try {
                            ...
                            if (isset($_POST['submit-2'])) {
                                ...
                                $sql2 = "SELECT DISTINCT `sdd`.`room_id`, `rd`.`room_name`, `rd`.`price`, `rd`.`available_beds`, 
                                            `rd`.`room_occupants`, `rd`.`status`, `rd`.`image`, `sdd`.`dorm_id`, `dd`.`dorm_address`
                                            FROM `student_dorm_data` AS sdd
                                            LEFT JOIN `dorm_data` AS dd ON `sdd`.`dorm_id` = `dd`.`dorm_id`
                                            LEFT JOIN `room_data` AS rd ON `sdd`.`room_id` = `rd`.`room_id`
                                            WHERE `sdd`.`room_id` LIKE '$search'
